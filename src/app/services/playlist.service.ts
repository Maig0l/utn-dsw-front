...
export class PlaylistService {
  ...
  getAllPlaylists(): Observable<Playlist[]> {
    return this.http.get<ApiResponse>(this.playlistEndpoint)
  ...
  updatePlaylist(id: number, name: string, description: string, is_private: boolean, owner:number, games:number): Observable<Playlist> {
    const url = this.playlistEndpoint + `/${id}`
    return this.http.put<Playlist>(url, {id, name, description, is_private, owner, games })
  }
