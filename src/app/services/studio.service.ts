...
export class StudioService {
    ...
    getAllStudios(): Observable<Studio[]> {
        return this.http.get<responseStudioList>(this.studiosEndpoint)
        .pipe(map((response) => response.data))
    }

    addStudio(name: string, type: string, site: string): Observable<Studio> {
        return this.http.post<responseStudio>(this.studiosEndpoint, { name, type, site })
          .pipe(map((response) => response.data))
    }

    deleteStudio(name: string): Observable<Studio> {
        const url = this.studiosEndpoint + `/${name}`;
        return this.http.delete<responseStudio>(url)
